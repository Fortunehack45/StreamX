...
export function Movies() {
  const { data: movies, loading } = useMedia({ type: 'movie', limit: 30 });
  
  const featuredMovie = movies[0];
  const trendingMovies = movies.slice(0, 4);
  const actionMovies = movies.filter(m => m.genres?.includes('Action')).slice(0, 4);
  const comedyMovies = movies.filter(m => m.genres?.includes('Comedy')).slice(0, 4);
  const sciFiMovies = movies.filter(m => m.genres?.includes('Science Fiction') || m.genres?.includes('Sci-Fi')).slice(0, 4);
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white p-4 md:p-12 pt-28 md:pt-40 font-sans pb-48 md:pb-24">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-6 auto-rows-min md:auto-rows-[300px] mb-12 md:mb-16">
        ...
        >
          {loading ? (
            ...
            <>
              <img src={featuredMovie?.backdrop_url || featuredMovie?.poster_url || "https://picsum.photos/seed/movie1/1200/800"} className="w-full h-full object-cover transition-soft duration-1000 group-hover:scale-105" referrerPolicy="no-referrer" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/95 via-black/20 to-transparent p-6 md:p-16 flex flex-col justify-end">
                <div className="flex gap-3 mb-4 md:mb-6">
                  <span className="px-4 py-1.5 rounded-full bg-white/10 backdrop-blur-xl text-[10px] font-black uppercase tracking-[0.2em] border border-white/5">{featuredMovie?.genres?.[0] || 'Movie'}</span>
                  <span className="px-4 py-1.5 rounded-full bg-white/10 backdrop-blur-xl text-[10px] font-black uppercase tracking-[0.2em] border border-white/5">{featuredMovie?.release_date?.split('-')[0]}</span>
                </div>
                <h2 className="text-4xl sm:text-5xl md:text-8xl font-black mb-6 md:mb-10 tracking-tighter leading-none uppercase">{featuredMovie?.title}</h2>
                <div className="flex flex-wrap items-center gap-4 md:gap-6">
                  <Link to={`/movies/${featuredMovie?.id}`} className="w-14 h-14 md:w-20 md:h-20 rounded-full bg-white text-black flex items-center justify-center hover:scale-110 transition-soft shadow-[0_0_60px_rgba(255,255,255,0.3)]">
      ...
      <div className="space-y-16 md:space-y-32">
        {[
          { title: "Trending", sub: "Cinema", data: trendingMovies },
          { title: "Action &", sub: "Adventure", data: actionMovies },
          { title: "Laugh Out", sub: "Loud", data: comedyMovies },
          { title: "Sci-Fi &", sub: "Fantasy", data: sciFiMovies },
        ].map((section) => (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 md:gap-8">
                {loading 
                  ...
                  : section.data.map((movie, i) => (
                      <MediaCard key={movie.id} media={movie} linkTo={`/movies/${movie.id}`} delay={i * 0.05} />
                    ))
                }
              </div>
            </section>
